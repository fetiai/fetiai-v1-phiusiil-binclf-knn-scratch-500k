  ...
  - name: fetiai-v1-phiusiil-binclf-knn-scratch-500k
    results:
      - task:
        ...
        metrics:
          - type: accuracy
            value: 0.9804494141946511
            name: Accuracy
          - type: precision
            value: 0.9813813813813814
            name: Phishing precision
          - type: recall
            value: 0.7592936802973977
            name: Phishing recall
---

<div align="center">

# fetiai-v1-phiusiil-binclf-knn-scratch-500k
...
**KNN (from scratch)** for phishing URL classification, served over HTTP as a single-model API.
...
`KNN (from scratch)` · `Trained on PhiUSIIL` · `SMOTE` · `Feature Engineering`
...
No training step and no download step: **the model is committed** (1.0 MB under
`model/`), so a fresh clone can serve immediately.

### With Docker

```bash
make docker-build && docker run -p 8000:8000 fetiai-v1-phiusiil-binclf-knn-scratch-500k:local
```
...
counterpart lives in [`fetiai-v1-phiusiil-binclf-knn-skl-500k`](../fetiai-v1-phiusiil-binclf-knn-skl-500k).
...
| Path | What it is |
|---|---|
| `model/knn_scratch.npz` | the trained model (500,001 stored values) |
...
The model itself is equally open — a NumPy `.npz` of the reference matrix and its labels.
Loading it executes no pickle and depends on no particular library version, which is the
practical difference between this repo and its scikit-learn counterpart in
[`fetiai-v1-phiusiil-binclf-knn-skl-500k`](../fetiai-v1-phiusiil-binclf-knn-skl-500k).
...
fitting is memorising — so `model/knn_scratch.npz` *is* the 10,000-row scaled reference set that
