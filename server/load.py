...
def load_model(model_dir: Path, manifest: dict[str, Any]) -> ScratchKNN:
    """k-NN keeps no learned parameters -- fitting is memorising the reference set."""
    reference = np.load(model_dir / MODEL_FILE)
    return ScratchKNN(k=int(reference["k"])).fit(reference["X"], reference["y"])
